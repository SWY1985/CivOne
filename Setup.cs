...
#else 
		private void PathFindingeMenu() => CreateMenu("Use smart PathFinding for \"goto\"", GotoMenu(PatchesMenu, 8),
		MenuItem.Create($"{true.YesNo()} (default)").OnSelect((s, a) => Settings.PathFinding = true).SetActive(() => !Settings.PathFinding),
			MenuItem.Create(false.YesNo()).OnSelect((s, a) => Settings.PathFinding = false).SetActive(() => Settings.PathFinding),
			MenuItem.Create("Back")
		);
#endif
