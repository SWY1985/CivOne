...
namespace CivOne
{
	public class Game : BaseInstance
	{
		...
		{
			IUnit unit = UnitsCreator.CreateUnit((Unit)type, x, y);
        ...
        public GameState GetGameState() => new GameState
        {
            ...
            Units = _units,
            GameTurn = GameTurn,
            CurrentPlayerNumber = PlayerNumber(HumanPlayer),
            Difficulty = Difficulty,
            Cities = GetCities().ToList(),
            Map = Map,
            AdvanceOrigin = _advanceOrigin,
            CityNames = _cityNames.ToList(),
            Settings = Settings,
        ...
        private void AddStartingUnits(byte player)
		{
			...
			while (loopCounter++ < 2000)
			{
				...
				else
				{
					...
					if (Common.TurnToYear((ushort)GameTurn) >= 0 && Map.ContinentTiles(tile.ContinentId).Any(t => t.City != null)) continue;
					
					Console.WriteLine(loopCounter.ToString());
				}
				
				// Starting position found, add Settlers
				IUnit unit = UnitsCreator.CreateUnit(Unit.Settlers, x, y);
				unit.Owner = player;
				_units.Add(unit);
